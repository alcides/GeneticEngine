...
class TestDepthing:
    def test_normal_depthing(self):
        ...
        y = random_node(r, g, 4, Root, method=PI_Grow().tree_init_method)
        g: Grammar = extract_grammar([Concrete, Middle, MiddleList], Root, False)
        z = random_node(r, g, 4, Root, method=PI_Grow().tree_init_method)
        g: Grammar = extract_grammar([ConcreteList, Middle], Root, False)
        a = random_node(r, g, 4, Root, method=PI_Grow().tree_init_method)
    ...
    def test_expansion_depthing(self):
        ...
        y = random_node(r, g, 8, Root, method=PI_Grow().tree_init_method)
        g: Grammar = extract_grammar([Concrete, Middle, MiddleList], Root, True)
        z = random_node(r, g, 8, Root, method=PI_Grow().tree_init_method)
